...
def load_units_qualities(dp,f=None,group_col=None,again=False):
    """
    ...
    if (dp / f).exists():
        ...
        if f"{group_col}" not in qualities.columns:
            print(
                f"WARNING there does not seem to be any group column in {f} - kilosort >2 weirdness. Making a fresh file."
            )
            qualities = generate_units_qualities(dp)
        else:
            if "unsorted" not in qualities[group_col].values and re_spikesorted:
        ...
        if regenerate:
            ...
            sorted_clusters = qualities.loc[qualities[group_col] != "unsorted", :]
...
def get_units(dp, quality="all", f=None,group_col=None,chan_range=None, again=False):
    ...
    if assert_multi(dp):
        ...
        qualities = load_units_qualities(dp, again=again,f=f,group_col=group_col)

    if quality == "all":
        units = qualities["cluster_id"].values
    else:
        quality_m = qualities[group_col] == quality
